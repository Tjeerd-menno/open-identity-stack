...
/// <summary>Runs the same authority races on SQLite by default, or an explicitly selected PostgreSQL server.</summary>
public sealed class AdministrativeAuthorityTestFixture : IAsyncLifetime
{
    ...
    {
        string? connection = Environment.GetEnvironmentVariable("OIS_AUTHORITY_TEST_POSTGRES");
        ...
        {
            Database = $"ois_authority_test_{Guid.NewGuid():N}",
            Pooling = false,
        };
