...
public sealed class CredentialCutoverResourceInventoryTests
{
    ...
    [Fact]
    public async Task ManagementUrisUseMigratorCaseInsensitiveDeduplication()
    {
        await using var connection = new SqliteConnection("DataSource=:memory:");
        await connection.OpenAsync();
        DbContextOptions<OpenIdentityStackDbContext> options = new DbContextOptionsBuilder<OpenIdentityStackDbContext>()
            .UseSqlite(connection).UseOpenIddict().Options;
        await using var db = new OpenIdentityStackDbContext(options);
        await db.Database.EnsureCreatedAsync();
        IConfiguration configuration = new ConfigurationBuilder().AddInMemoryCollection(new Dictionary<string, string?>
        {
            ["OpenIddict:Clients:ManagementWeb:RedirectUris:0"] = "https://console.example/auth/callback",
            ["OpenIddict:Clients:ManagementWeb:RedirectUris:1"] = "HTTPS://CONSOLE.EXAMPLE/AUTH/CALLBACK",
            ["OpenIddict:Clients:ManagementWeb:PostLogoutRedirectUris:0"] = "https://console.example/",
            ["OpenIddict:Clients:ManagementWeb:PostLogoutRedirectUris:1"] = "HTTPS://CONSOLE.EXAMPLE/"
        }).Build();
        IHostEnvironment environment = Substitute.For<IHostEnvironment>();
        environment.EnvironmentName.Returns("Production");
        IDateTimeProvider clock = Substitute.For<IDateTimeProvider>();
