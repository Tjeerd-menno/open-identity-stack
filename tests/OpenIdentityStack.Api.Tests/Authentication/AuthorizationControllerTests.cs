...
#pragma warning restore CA2012
    ...
    [Fact]
    public async Task Introspect_WhenAuthFails_ReturnsChallenge()
    {
        // Arrange
        var request = new OpenIddictRequest { ClientId = "patient-api" };
        this._requestService.GetRequest(Arg.Any<HttpContext>()).Returns(request);
        this.SetupMockServices(null, authSuccess: false);

        // Act
        IActionResult result = await this._controller.Introspect();

        // Assert
        ChallengeResult challenge = Assert.IsType<ChallengeResult>(result);
        Assert.Contains(OpenIddictServerAspNetCoreDefaults.AuthenticationScheme, challenge.AuthenticationSchemes);
        Assert.Equal(
            OpenIddictConstants.Errors.InvalidToken,
            challenge.Properties!.Items[OpenIddictServerAspNetCoreConstants.Properties.Error]);
    }

    [Fact]
    public async Task Introspect_ReturnsActiveSubjectAndCallerFilteredFreshPermissions()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var request = new OpenIddictRequest { ClientId = "patient-api" };
        ...
        [
            ...
            new Claim("permission", "inventory-api:read-stock")
        ];
        var identity = new ClaimsIdentity(claims, OpenIddictServerAspNetCoreDefaults.AuthenticationScheme);
        var principal = new ClaimsPrincipal(identity);
        this.SetupMockServices(principal);
        ...
        this._getUserEffectiveRolesQueryHandler.HandleAsync(
        ...
        Dictionary<string, object> response = Assert.IsType<Dictionary<string, object>>(ok.Value);
        Assert.True((bool)response["active"]);
        Assert.Equal(userId.ToString(), response[OpenIddictConstants.Claims.Subject]);
        IReadOnlyList<string> permissions = Assert.IsAssignableFrom<IReadOnlyList<string>>(response["permissions"]);
        permissions.ShouldBe(["patient-api:read-patients", "patient-api:write-patients"]);
    }
