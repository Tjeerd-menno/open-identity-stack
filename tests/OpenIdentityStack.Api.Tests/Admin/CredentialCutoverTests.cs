...
public sealed class CredentialCutoverTests
{
    ...
    public async Task LegacyRehearsalPreservesQuarantineAndDisablementDespiteTestedEmergencyLogin()
    {
        ...
        HttpResponseMessage firstResponse = await human.GetAsync("/api/admin/security/cutover-readiness");
        string firstJson = await firstResponse.Content.ReadAsStringAsync();
        JsonObject firstPayload = JsonNode.Parse(firstJson)!.AsObject();
        firstPayload["emergencyAccess"]!.AsObject().ContainsKey("sessionId").ShouldBeFalse();
        CredentialCutoverPreflight first = JsonSerializer.Deserialize<CredentialCutoverPreflight>(firstJson, JsonSerializerOptions.Web)!;
