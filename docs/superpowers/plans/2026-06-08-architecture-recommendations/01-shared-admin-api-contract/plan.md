...
**Goal:** Deepen `src/frontend-packages/admin-api-client` from a fetch-like transport module into the shared Admin API contract used by Management Web and Management Web.
...
**Tech Stack:** TypeScript 6, Vite, Vitest, React Query callers, `@openidentitystack/admin-api-client`, Management Web, Management Web.
...
- Modify: `src/OpenIdentityStack.ManagementWeb/src/features/*/api/*-api.ts` to re-export shared contract functions behind existing names where needed.
- Modify: `src/OpenIdentityStack.ManagementWeb/vite.config.ts`, `src/OpenIdentityStack.ManagementWeb/vite.config.ts`, and TS configs only if the current alias cannot resolve the package cleanly.
...
- Modify: `src/OpenIdentityStack.ManagementWeb/src/features/users/api/users-api.ts`
...
- [ ] Run `cd src/OpenIdentityStack.ManagementWeb; npm test -- src/features/users/api/users-api.test.ts`.
- [ ] Commit: `git add src/frontend-packages/admin-api-client src/OpenIdentityStack.ManagementWeb/src/features/users src/OpenIdentityStack.ManagementWeb/src/features/users && git commit -m "Move users admin api contract to shared client"`.
...
- Modify: `src/OpenIdentityStack.ManagementWeb/src/features/groups/api/groups-api.ts`
...
- [ ] Replace URLSearchParams construction in Management Web with shared params handling.
- [ ] Run shared, Management Web, and Management Web group API tests.
- [ ] Commit: `git add src/frontend-packages/admin-api-client/src/groups.ts src/frontend-packages/admin-api-client/src/groups.test.ts src/OpenIdentityStack.ManagementWeb/src/features/groups src/OpenIdentityStack.ManagementWeb/src/features/groups && git commit -m "Move groups admin api contract to shared client"`.
...
- Modify: `src/OpenIdentityStack.ManagementWeb/src/features/*/api/*-api.test.ts`
...
- [ ] Commit: `git add src/OpenIdentityStack.ManagementWeb src/OpenIdentityStack.ManagementWeb src/frontend-packages/admin-api-client && git commit -m "Consolidate admin api contract tests"`.

## Validation

- `cd src/frontend-packages/admin-api-client; npm test`
- `cd src/OpenIdentityStack.ManagementWeb; npm run type-check && npm test`
- `cd src/OpenIdentityStack.ManagementWeb; npm run type-check && npm test -- --run`
