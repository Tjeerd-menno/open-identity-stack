...
The first release enforces the [boundary decision](../adr/0005-identity-and-administrative-trust-boundaries.md) from initial deployment. Development credentials issued before these restrictions are not production credentials. Downgrading to a version that infers authority from role names or permits ungoverned grants reopens those boundaries.
