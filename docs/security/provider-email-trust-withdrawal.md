...
The local-session boundary from ticket 11 must be deployed with this lifecycle: every authentication cookie use and authorization issuance must reject a revoked or missing session. This ticket marks sessions revoked; that boundary enforces the marker and prevents a surviving browser cookie from silently creating a replacement credential. Fresh independent authentication may create a new session with corrected claims.
