...
export function AdministrativeApprovalDialog({ onReauthenticate }: { onReauthenticate: () => Promise<void> }) {
  ...
  return (
    <Modal opened={request !== null} onClose={() => close(false)} title="Approve unrestricted access" centered>
      <Stack>
        <Text>This operation can grant every current and future platform permission. Reserve unrestricted access for controlled administration and emergency recovery.</Text>
        {requiresAuthentication ? (
          ...
          <Checkbox checked={acknowledged} onChange={(event) => setAcknowledged(event.currentTarget.checked)}
            label="I acknowledge the unrestricted access this operation grants." />
        )}
