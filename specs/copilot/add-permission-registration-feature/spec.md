...
**Independent Test**: Can be fully tested by adding, editing, deprecating, and attempting to remove permissions for a registered service while observing how existing role assignments and administrative views are affected.

**Acceptance Scenarios**:

1. **Given** a registered service, **When** the owning service owner adds a new valid permission, **Then** the permission is available for future RBAC assignments and is shown as active in administrative views.
2. **Given** an active permission already assigned to one or more roles, **When** the service owner marks it as deprecated, **Then** existing assignments remain visible, new assignment attempts are clearly discouraged or blocked according to policy, and administrators can identify impacted roles.
3. **Given** a permission that is assigned to roles or otherwise in active use, **When** the service owner attempts to delete it, **Then** the deletion is blocked unless the permission is first removed from all dependent assignments or explicitly retired through the approved lifecycle.
...
**Independent Test**: Can be fully tested by registering permissions for multiple services, assigning those permissions to roles, reviewing assignments, and confirming unavailable or retired permissions are handled safely.

**Acceptance Scenarios**:

1. **Given** multiple services have registered active permissions, **When** an administrator creates or edits a role, **Then** they can select permissions grouped or filterable by service and permission status.
2. **Given** a permission is disabled or retired, **When** an administrator reviews roles, **Then** the affected assignments are highlighted with guidance for replacement or removal.
...
- A permission is deprecated, retired, or restored after administrators have already used it in role definitions.
...
- **FR-003**: The system MUST allow authorized service owners or administrators to declare one or more permissions for a registered service, including a stable permission key, display name, description, status, and intended use.
...
- **FR-007**: The system MUST allow authorized service owners to update non-identity metadata for permissions they own, such as display name, description, documentation reference, and status.
...
- **FR-009**: The system MUST support a permission lifecycle with at least active, deprecated, disabled, and retired states.
...
- **FR-011**: The system MUST show dependency information before a permission is disabled, retired, or removed, including impacted roles and administrative assignments.
- **FR-012**: The system MUST distinguish between permissions that can be assigned to new roles and permissions retained only for historical, migration, or existing-assignment visibility.
- **FR-013**: The system MUST expose registered active permissions to RBAC role creation and role editing workflows so administrators can assign permissions by service.
...
- **FR-015**: The system MUST clearly indicate deprecated, disabled, retired, or unavailable permissions in role management, access review, and administrative views.
...
- **FR-018**: The system MUST record audit events for service registration, permission creation, permission update, status change, ownership change, denied update attempts, and validation failures.
...
- **FR-023**: The system MUST maintain historical visibility of previously registered permissions that have been assigned or audited, even after those permissions are deprecated, disabled, or retired.
- **FR-024**: The system MUST provide a safe way to recover from accidental deprecation or disabling of a permission when policy permits restoration.
...
- **Service Permission**: A permission exposed by a registered service. Key attributes include stable permission key, service association, display name, description, intended use, lifecycle status, assignability, creation date, last updated date, and historical visibility.
...
- Removing a permission from new use is safer than deleting it, so deprecation or retirement is the default lifecycle path for permissions that may have been assigned.
...
- **SC-002**: 100% of accepted permission registrations include a stable service identifier, stable permission key, owner, status, and audit record.
...
- **SC-005**: When a permission is deprecated, disabled, or retired, administrators can identify impacted roles and assignments within 1 minute.
- **SC-006**: Existing RBAC assignments remain visible and attributable for 100% of permissions that have been deprecated, disabled, or retired.
