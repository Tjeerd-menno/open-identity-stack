...
### Phase 8 Providers Completion Note

As of 2026-06-02, the Providers slice is implemented in ManagementWeb with Mantine components under `src/OpenIdentityStack.ManagementWeb/src/features/providers/`. It preserves `/providers`, `/providers/new`, `/providers/:id`, and `/providers/:id/edit`, ports the AdminWeb operator workflow for OIDC provider list/search/create/detail/edit/enable/disable/delete, gates write/status actions with `providers:write`, deletion with `providers:delete`, and route access with `providers:read`, and uses the existing backend provider contract: `GET /api/admin/providers?includeDisabled=true`, `GET /api/admin/providers/{id}`, `POST /api/admin/providers`, `PATCH /api/admin/providers/{id}`, `POST /api/admin/providers/{id}/enable`, `POST /api/admin/providers/{id}/disable`, and `DELETE /api/admin/providers/{id}`. It is covered by Vitest API/component/route tests plus .NET/xUnit Playwright E2E coverage. Verification evidence: `npm test -- src/features/providers/providers-api.test.ts src/features/providers/ProvidersPage.test.tsx src/routes/AppRoutes.test.tsx` passed 26 tests; `npm run type-check` passed; `dotnet test --project tests\OpenIdentityStack.ManagementWeb.E2ETests\OpenIdentityStack.ManagementWeb.E2ETests.csproj --no-restore -- --filter-namespace OpenIdentityStack.ManagementWeb.E2ETests` passed 8 tests.

### Phase 9 Settings Completion Note

As of 2026-06-02, the Settings slice is implemented in ManagementWeb with Mantine components under `src/OpenIdentityStack.ManagementWeb/src/features/settings/`. It preserves `/settings`, ports the AdminWeb authentication settings workflow for viewing active providers, changing the default authentication provider, and toggling admin local fallback, gates route access with `system:settings`, and uses the existing backend settings contract: `GET /api/admin/authentication-settings`, `GET /api/admin/authentication-settings/providers`, `PUT /api/admin/authentication-settings/default-provider`, and `PUT /api/admin/authentication-settings/local-fallback`. It is covered by Vitest API/component/route tests plus .NET/xUnit Playwright E2E coverage. Verification evidence: `npm test -- src/features/settings/settings-api.test.ts src/features/settings/SettingsPage.test.tsx src/routes/AppRoutes.test.tsx` passed 26 tests; `npm run type-check` passed; `dotnet test --project tests\OpenIdentityStack.ManagementWeb.E2ETests\OpenIdentityStack.ManagementWeb.E2ETests.csproj --no-restore -- --filter-namespace OpenIdentityStack.ManagementWeb.E2ETests` passed 9 tests.

### Phase 10 Application Permissions Completion Note
