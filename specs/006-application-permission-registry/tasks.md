...
**Tests**: Required before implementation in each slice. Security-sensitive externally visible behavior requires API/integration coverage. Project-owned `/api/admin/*` changes require contract tests. Backend and AdminWeb ship together per slice.
...
- [X] T014 [P] [S1] Add AdminWeb unit/API-client tests for manifest editor validation and API errors.
- [X] T015 [S1] Add AdminWeb E2E for create from inline manifest, detail view, newer non-destructive update, and same/older version validation.
...
- [X] T025 [S1] Implement AdminWeb `Application Permissions` nav, list, detail, create, raw JSON editor, structured editor, maintainer management, ownership transfer, enable/disable, and manifest update flows.
...
- [X] T036 [S2] Add AdminWeb E2E for dynamic wildcard assignment and one platform broad-grant acknowledgement flow.
...
- [X] T046 [S2] Implement AdminWeb unified role picker with source/kind display and broad-grant acknowledgement.
...
- [X] T054 [S3] Add AdminWeb E2E for one representative destructive manifest omission preview/apply flow.
...
- [X] T064 [S3] Implement AdminWeb destructive preview/confirmation/result flows.
...
- [X] T072 [S4] Add AdminWeb E2E for preview/apply from controlled local fixture endpoint.
...
- [X] T082 [S4] Implement AdminWeb remote import UI.
...
- [X] T102 [S5] Implement integrity diagnostics and AdminWeb diagnostics/remediation view.

## Phase 7: Cleanup And Verification

- [X] T110 Run `dotnet build OpenIdentityStack.slnx --no-restore`.
- [X] T111 Run focused .NET tests for all completed slices.
- [X] T112 Run AdminWeb `npm run build`, `npm run lint`, and `npm test`.
- [X] T113 Run focused AdminWeb E2E for completed slices.
