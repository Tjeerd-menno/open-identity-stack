...
**Testing**: xUnit-style .NET tests through Microsoft.Testing.Platform, contract tests for `/api/admin/*`, Vitest for Management Web units, Playwright/Management Web E2E for vertical flows

**Target Platform**: Current API/Management Web deployment through Aspire/local containers and existing production container model

**Project Type**: Full-stack IAM/admin feature with backend API, persistence, authorization, token/introspection behavior, and Management Web workflows
...
| II. Test-First, Risk-Based Verification | PASS | Security-sensitive external behavior requires API/integration tests; project-owned admin APIs require contract tests; Management Web flows require focused E2E. |
| III. Layered Architecture with Vertical Feature Slices | PASS | Feature is explicitly sliced end-to-end; Application owns orchestration/ports; Infrastructure owns EF transactions; Api owns Minimal API endpoint mappers; Management Web ships with backend slices. |
...
| V. Operational Reliability and Observability | PASS | Destructive changes are previewed in Management Web, recalculated by API, transactional, and audited with details. Remote fetches are bounded and trusted. |
| VI. User-Facing and API Consistency | PASS | Management Web navigation is `Application Permissions`; role response `permissions: string[]` remains stable; catalogs keep platform and dynamic sources distinct. |
...
**Structure Decision**: Keep registry domain rules in Domain, orchestration and ports in Application, EF repositories/transactions/assignment-store implementations in Infrastructure, Minimal API endpoint mappers in Api, and workflow UI under Management Web. Management Web and backend are implemented together by vertical slice.
...
- Management Web `Application Permissions` navigation, list/detail/create/update, raw JSON and structured manifest editor.
...
- Unified Management Web role picker.
...
- API/integration tests for introspection filtering/expansion; Management Web E2E for wildcard assignment flow.
...
- Management Web destructive preview and confirmation flows.
...
- Management Web remote import UI.
...
See [decision-log.md](decision-log.md). Research/grilling resolved the primary semantic risks: permission identity, manifest completeness and versioning, wildcard derivation and expansion, assignment cleanup, transaction/audit boundaries, ownership, remote fetch trust, and Management Web slice scope.
...
| II. Test-First, Risk-Based Verification | PASS | Slices require API/integration, contract, unit, and Management Web E2E tests before implementation. |
| III. Layered Architecture with Vertical Feature Slices | PASS | Plan uses five backend+Management Web vertical slices. |
...
| VI. User-Facing and API Consistency | PASS | Management Web label and API terminology are clarified; existing role permission string contract is preserved. |
